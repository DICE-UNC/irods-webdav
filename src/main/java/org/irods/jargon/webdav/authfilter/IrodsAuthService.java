...
public class IrodsAuthService {
	...
	private static final Logger log = LoggerFactory.getLogger(IrodsAuthService.class);

	/**
	 * Get the response that was cached via the basic auth process
	 * 
	 * @param userName
	 * @param password
	 * @return
	...
	public AuthResponse getCached(String userName, String password) {
		...
		if (cached.getAuthenticatingIRODSAccount().getUserName().equals(userName)
			...
			throw new WebDavRuntimeException("auth cache invalidation error");
		}
		return cached;
	}
	...
	public AuthResponse authenticate(final String userName, final String password)
			throws AuthenticationException, WebDavException {
		...
		/*
		 * log.debug("look in cache for cached login"); AuthResponse cached =
		 * authResponseCache.get();
		 * 
		 * if (cached != null) { log.debug("in thread local cache"); if
		 * (!cached.getAuthenticatingIRODSAccount().getUserName()
		 * .equals(userName)) { log.warn("cache is not same as user name"); }
		 * else { return cached; } }
		 */
		...
		if (irodsAccessObjectFactory == null) {
			throw new ConfigurationRuntimeException("null irodsAccessObjectFactory");
		}

		log.debug("authenticating:{}", irodsAccount);
		try {
			AuthResponse response = irodsAccessObjectFactory
					.authenticateIRODSAccountUtilizingCachedConnectionIfPresent(irodsAccount);
			if (response.getAuthenticatingIRODSAccount().getPassword().equals(irodsAccount.getPassword())) {
				log.info("verified, can use cache");
			} else {
				log.error("authentication exception invalid auth");
				throw new AuthenticationException("invalid cached password");
			}
	 ...
	 * @throws JargonException
	 */
	public IRODSAccount getIrodsAccountFromAuthValues(final String userName, final String password)
			throws JargonException {
		...
		if (webDavConfig == null) {
			throw new ConfigurationRuntimeException("webDavConfig not available");
		}

		AuthScheme authScheme;
		if (webDavConfig.getAuthScheme() == null || webDavConfig.getAuthScheme().isEmpty()) {
			log.debug("unspecified authScheme, use STANDARD");
			authScheme = AuthScheme.STANDARD;
		} else if (webDavConfig.getAuthScheme().equals(AuthScheme.STANDARD.toString())) {
			log.debug("using standard auth");
			authScheme = AuthScheme.STANDARD;
		} else if (webDavConfig.getAuthScheme().equals(AuthScheme.PAM.toString())) {
			log.debug("using PAM");
			authScheme = AuthScheme.PAM;
		} else {
			log.error("cannot support authScheme:{}", webDavConfig);
			throw new ConfigurationRuntimeException("unknown or unsupported auth scheme");
		}

		return IRODSAccount.instance(webDavConfig.getHost(), webDavConfig.getPort(), userName, password, "",
				webDavConfig.getZone(), webDavConfig.getDefaultStorageResource(), authScheme);
	}
	 ...
	 *         Jargon
	 */
	public static IRODSAccount retrieveIrodsAccountFromAuthResponse(final AuthResponse authResponse) {
